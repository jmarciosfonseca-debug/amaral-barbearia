...
// 🔧 Nova: promo resgatada aparece na ficha
// 🔧 Nova: ao clicar na promo → agendamento direto (pula serviço/valor)
...
function Estrelas({ nota, size = 16 }) {
  return (
    <div style={{ display:'flex', gap:'2px' }}>
      {[1,2,3,4,5].map(i => <span key={i} style={{ fontSize:size, color:i<=nota?'#E8C96A':'#3A2018' }}>★</span>)}
    </div>
  );
}

// ─────────────────────────────────────────────────────────────
// AGENDAMENTO DIRETO DA PROMO — só barbeiro + horário
// ─────────────────────────────────────────────────────────────
function AgendarPromo({ cliente, promo, onConcluir, onBack, dark }) {
  ...
  const diasDisponiveis = React.useMemo(() => {
    ...
    for (let i = 0; i < 14; i++) {
      ...
      // Se validade 'hoje', só o dia de hoje
      if (promo.validade === 'hoje' && i > 0) break;
      // Se tem expiração, não passa dela
      if (exp && d > exp) break;
      dias.push(dStr);
    }
    return dias;
  ...
  async function confirmar() {
    ...
    try {
      ...
      // Marca promo como utilizada na ficha
      await updateDoc(doc(db,'resgates_promo',`${cliente.cpf}_${promo.id||'ativa'}`), { utilizado: true, utilizadoEm: serverTimestamp() });
      // Notifica barbearia
    ...
    <div style={{ ...s.app, padding:'20px', paddingBottom:'100px' }}>
      ...
      {barbeiros.map(b => (
        <div key={b.id} onClick={() => { setBarbeiro(b); setDataSel(null); setHoraSel(null); }}
          ...
          <div style={{ width:'48px', height:'48px', borderRadius:'50%', background:'linear-gradient(135deg,#5C2218,#8B3A2A)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'18px', fontWeight:'700', color:'#F5EFE6', border:b.disponivel?'2px solid #4CAF50':'2px solid #FFC107', overflow:'hidden' }}>
        ...
        <>
          ...
          <div style={{ overflowX:'auto', display:'flex', gap:'8px', paddingBottom:'8px', marginBottom:'16px' }}>
            {diasDisponiveis.map(data => {
              ...
              return (
                <div key={data} onClick={() => setDataSel(data)} style={{ minWidth:'56px', padding:'10px 8px', borderRadius:'12px', textAlign:'center', cursor:'pointer', background:sel?'#8B3A2A':'#231410', border:sel?'1.5px solid #E8C96A':'1px solid #3A2018' }}>
      ...
      {barbeiro && dataSel && horaSel && (
        <div style={{ position:'fixed', bottom:0, left:'50%', transform:'translateX(-50%)', width:'100%', maxWidth:'430px', background:'#1A0F0D', borderTop:'1px solid #3A2018', padding:'12px 16px 28px', zIndex:100 }}>
          ...
          <button onClick={confirmar} disabled={salvando} style={{ width:'100%', padding:'14px', borderRadius:'14px', border:'none', background:'linear-gradient(135deg,#8B0000,#F44336)', color:'#fff', fontSize:'15px', fontWeight:'700', cursor:salvando?'wait':'pointer' }}>
...
export default function PerfilCliente({ cliente, onBack, dark, onNavegar }) {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      // Agendamentos
      const qA = query(collection(db,'agendamentos'), where('clienteCpf','==',chave), orderBy('data','desc'), orderBy('hora','desc'));
      ...
      unsubAv = onSnapshot(qAv, snap => setAvaliacoes(snap.docs.map(d=>({id:d.id,...d.data()}))), ()=>{});

      // Assinatura
      const qAs = query(collection(db,'assinaturas'), where('clienteId','==',chave));
      unsubAs = onSnapshot(qAs, snap => {
        const todas = snap.docs.map(d=>({id:d.id,...d.data()}));
        setAssinatura(todas.find(a=>a.status==='ativa') || todas.find(a=>a.status==='pendente_pagamento') || null);
      }, ()=>{});
  ...
  function calcularSaldo() {
    ...
    return (assinatura.servicosInclusos||[]).map(sv => ({ nome:sv, usado:assinatura.usoServicos?.[sv]||0, limite:assinatura.limiteServicos?.[sv]||4 }));
  }
  ...
  return (
    <div style={{ ...s.app, paddingBottom:'80px' }}>
      ...
      <div style={{ background:'linear-gradient(135deg,#1A0F0D,#2E1A14)', position:'relative' }}>
        ...
        <div style={{ position:'relative', display:'inline-block', marginLeft:'20px', marginTop:'-36px' }}>
          <label style={{ cursor:editando?'pointer':'default' }}>
            ...
            {editando&&<input type="file" accept="image/*" onChange={e=>{ const f=e.target.files[0]; if(f) comprimirFoto(f,b=>setFoto(b)); }} style={{ display:'none' }} />}
          ...
          {editando ? (
            <div>
              ...
              <div style={{ display:'flex', gap:'8px', marginTop:'10px' }}>
                <button onClick={salvarPerfil} disabled={salvando} style={{ flex:1, background:'linear-gradient(135deg,#5C2218,#8B3A2A)', border:'none', borderRadius:'10px', padding:'10px', color:'#F5EFE6', fontWeight:'700', fontSize:'13px', cursor:'pointer' }}>{salvando?'Salvando...':'✅ Salvar'}</button>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'8px', padding:'12px 16px', borderBottom:`1px solid ${border}` }}>
        {[{label:'Cortes',value:concluidos.length,color:'#E8C96A'},{label:'Total gasto',value:`R$${totalGasto.toFixed(0)}`,color:'#4CAF50'},{label:'Barbeiro fav.',value:barbeiroFav||'—',color:'#2E7D7A',small:true}].map(item => (
      ...
      <div style={{ padding:'16px' }}>
                ...
                <div style={{ padding:'14px' }}>
                  <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'10px' }}>
                    ...
                    <div style={{ flex:1 }}>
                      ...
                      <div style={{ fontSize:'11px', color:assinatura.status==='ativa'?'#4CAF50':'#FFC107' }}>{assinatura.status==='ativa'?'✅ Ativo':'⏳ Aguardando'} · R$ {Number(assinatura.preco).toFixed(2)}/mês</div>
                    </div>
                    <button onClick={()=>onNavegar&&onNavegar('assinatura_plano')} style={{ background:'transparent', border:`1px solid ${border}`, borderRadius:'8px', padding:'4px 10px', fontSize:'11px', color:textSub, cursor:'pointer' }}>Ver</button>
                  </div>
                        ...
                        return (
                          <div key={sv.nome} style={{ marginBottom:'6px' }}>
                            <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'3px' }}>
                              ...
                              <span style={{ fontSize:'11px', color:restante===0?'#F44336':assinatura.planoCor||'#C9A84C', fontWeight:'700' }}>{restante===0?'Esgotado':restante+' restante'+(restante!==1?'s':'')}</span>
                            </div>
        ...
        {aba==='historico' && (
          <div>
            ...
            {!loading&&agendamentos.length===0&&<div style={{ textAlign:'center', padding:'40px', color:textSub }}><div style={{ fontSize:'32px', marginBottom:'8px' }}>📋</div><div>Nenhum agendamento encontrado.</div></div>}
            {cancelados.length>0&&<div style={{ background:'rgba(255,193,7,0.08)', border:'1px solid rgba(255,193,7,0.2)', borderRadius:'10px', padding:'10px 12px', marginBottom:'12px', fontSize:'11px', color:'#FFC107' }}>💡 Você pode excluir cancelamentos do histórico.</div>}
            {agendamentos.map(ag => {
                ...
                <div key={ag.id} style={{ background:cardBg, border:`1px solid ${cancelado?'#3A101044':ag.status==='concluido'?'#2E7D7A44':border}`, borderRadius:'14px', padding:'14px', marginBottom:'10px', opacity:cancelado?0.8:1 }}>
                  <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'6px' }}>
                    ...
                    <div style={{ textAlign:'right', display:'flex', flexDirection:'column', alignItems:'flex-end', gap:'4px' }}>
                      ...
                      {cancelado&&<button onClick={()=>ocultarCancelamento(ag)} style={{ background:'rgba(244,67,54,0.1)', border:'none', borderRadius:'6px', padding:'3px 8px', fontSize:'10px', color:'#F44336', cursor:'pointer', fontWeight:'600' }}>🗑️ Excluir</button>}
                    </div>
                  </div>
                </div>
              );
        ...
        {aba==='avaliacoes' && (
          <div>
            {avaliacoes.length===0?(
              <div style={{ textAlign:'center', padding:'40px', color:textSub }}><div style={{ fontSize:'32px', marginBottom:'8px' }}>⭐</div><div style={{ marginBottom:'4px', fontWeight:'600', color:textMain }}>Nenhuma avaliação ainda</div></div>
            ):(
              <>
