...
// ─────────────────────────────────────────────────────────────
// Passo 10: Painel da Recepção
// - Fila do dia em tempo real
// - Confirmar pagamento (Dinheiro / Cartão / Pix)
// - Criar agendamento manual
// - Visão geral da barbearia
// Lê/escreve em: Firestore /agendamentos
// ─────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────
function ModalNovoAgendamento({ onSalvar, onFechar }) {
  ...
  React.useEffect(() => {
    ...
    getDocs(collection(db, 'barbeiros')).then(snap => {
      setBarbeiros(snap.docs.map(d => ({ id: d.id, ...d.data() })).filter(b => b.ativo));
    });
    // Carregar serviços
      ...
      <div style={{
        ...
        <div style={{ marginBottom: '12px' }}>
          ...
          <select style={{ ...inputStyle }} value={form.servico} onChange={e => handleServicoChange(e.target.value)}>
            <option value="">Selecione...</option>
            {servicos.map(s => <option key={s.id} value={s.nome}>{s.nome} — R$ {s.valor.toFixed(2).replace('.', ',')}</option>)}
          </select>
        </div>
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '12px' }}>
          ...
          <div style={{ flex: 1 }}>
            <label style={{ fontSize: '11px', color: '#9A8880', display: 'block', marginBottom: '5px' }}>Horário</label>
        ...
        <div style={{ marginBottom: '14px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            {[
              ...
              <button
                key={f.id}
                onClick={() => setField('pagamento', f.id)}
                style={{
...
// ─────────────────────────────────────────────────────────────
function CardFila({ ag, onPagar, onConcluir }) {
...
export default function PainelRecepcao({ onBack, dark }) {
  ...
  const [aba, setAba]                       = React.useState('fila'); // fila | resumo

  // ── Escuta agendamentos de hoje ───────────────────────────
  React.useEffect(() => {
    const q = query(
    ...
    <div style={{ ...s.app, paddingBottom: '40px' }}>
      ...
      {modalPagamento && (
        <ModalPagamento
          ...
          onConfirmar={() => setModalPagamento(null)}
          onFechar={() => setModalPagamento(null)}
        />
      )}
      {modalNovo && (
        <ModalNovoAgendamento
          onSalvar={() => setModalNovo(false)}
