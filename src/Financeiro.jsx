...
// ─────────────────────────────────────────────────────────────
// Passo 9: Financeiro (Gerente)
// - Resumo do dia: total, por barbeiro, por forma de pagamento
// - Navegação entre dias
// - Lista de agendamentos concluídos
// - Comparativo entre barbeiros
// Lê de: Firestore /agendamentos
// ─────────────────────────────────────────────────────────────
...
export default function Financeiro({ onBack, dark }) {
  ...
  const [aba, setAba]                 = React.useState('dia'); // dia | barbeiros | historico

  // ── Escuta agendamentos do dia em tempo real ──────────────
  React.useEffect(() => {
    setCarregando(true);
    const q = query(
  ...
  const ativos = agendamentos.filter(a =>
    a.status !== 'cancelado_cliente' && a.status !== 'cancelado_barbeiro'
  );
  ...
  // Próximos 7 dias
  const dias = Array.from({ length: 7 }, (_, i) => addDias(hoje(), i - 3)).filter(d => d >= addDias(hoje(), -6));
  ...
  return (
    <div style={{ ...s.app, paddingBottom: '40px' }}>
        ...
        {Array.from({ length: 14 }, (_, i) => addDias(hoje(), i - 7)).map(data => {
          ...
          return (
            ...
            >
              ...
              <div style={{ fontSize: '16px', fontWeight: '700', color: '#F5EFE6', marginTop: '2px' }}>
                {d.getDate()}
              </div>
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid #3A2018' }}>
        {[
          { id: 'dia',       label: '📊 Resumo'    },
          { id: 'barbeiros', label: '✂️ Barbeiros'  },
          { id: 'lista',     label: '📋 Detalhes'   },
        ].map(a => (
          <button
            key={a.id}
            onClick={() => setAba(a.id)}
            style={{
              ...
              fontSize: '12px', fontWeight: aba === a.id ? '700' : '400',
          ...
          <>
            ...
            {aba === 'dia' && (
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '16px' }}>
                  {[
                    ...
                    <div key={item.label} style={{
                      ...
                      gridColumn: item.big ? 'span 2' : 'span 1',
                    }}>
                      <div style={{ fontSize: '10px', color: '#9A8880', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '1px' }}>
                        {item.label}
                      </div>
                      <div style={{ fontSize: item.big ? '28px' : '22px', fontWeight: '700', color: item.color }}>
                        {item.value}
                      </div>
                    </div>
                  ))}
                ...
                {/* Formas de pagamento */}
                <div style={{ fontFamily: "'Playfair Display', serif", fontSize: '15px', color: '#F5EFE6', marginBottom: '12px' }}>
                  Formas de pagamento
                </div>
                <div style={{ background: '#231410', borderRadius: '14px', border: '1px solid #3A2018', overflow: 'hidden', marginBottom: '16px' }}>
                  {[
                    ...
                    <div key={item.label} style={{
                      padding: '14px 16px',
                      borderBottom: i === 0 ? '1px solid #3A2018' : 'none',
                    }}>
                ...
                {totalSinais > 0 && (
                  <div style={{
                    ...
                    <div style={{ fontSize: '11px', color: '#9A8880', marginTop: '4px' }}>
                      Já garantidos, serão abatidos no total
                    </div>
                  </div>
                )}
                ...
                {ativos.length === 0 && (
                  <div style={{ textAlign: 'center', padding: '40px', color: '#9A8880' }}>
                    ...
                    <div style={{ fontSize: '14px' }}>Nenhum agendamento neste dia</div>
                  </div>
                )}
              </div>
            )}
                    ...
                    <div key={b.nome} style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
                        <div style={{
                          ...
                          background: i === 0 ? 'linear-gradient(135deg, #A07830, #E8C96A)'
                            : i === 1 ? 'linear-gradient(135deg, #888, #C0C0C0)'
                            : 'linear-gradient(135deg, #5C2218, #8B3A2A)',
                          display: 'flex', alignItems: 'center', justifyContent: 'center',
                          fontWeight: '700', fontSize: '16px', color: '#0A0A0A',
                          flexShrink: 0,
                        }}>
                          {i === 0 ? '🥇' : i === 1 ? '🥈' : '🥉'}
                        </div>
                      ...
                      <div style={{ height: '4px', background: '#2E1A14', borderRadius: '2px' }}>
                        <div style={{
                          height: '100%',
                          width: totalGeral ? `${(b.total / totalGeral * 100)}%` : '0%',
                          background: 'linear-gradient(90deg, #8B3A2A, #E8C96A)',
                ...
                {agendamentos.length === 0 ? (
                  ...
                  [...agendamentos]
                    ...
                    .map(ag => {
                      ...
                      return (
                        <div key={ag.firestoreId} style={{
                          background: '#231410', borderRadius: '14px',
                          border: cancelado ? '1px solid #3A1010' : '1px solid #3A2018',
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                              ...
                              <span style={{ fontSize: '13px', color: '#F5EFE6', fontWeight: '600' }}>
                          ...
                          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                            ...
                            <span style={{
                              ...
                              color: ag.status === 'concluido' ? '#2E7D7A'
                                : cancelado ? '#F44336' : '#4CAF50',
                            }}>
