...
// ✅ SEM pré-agendamento em qualquer parte do código
// ✅ Deep Link via URL params (?barbeiro=&data=&hora=)
// ✅ Ficha pública do barbeiro acessível da Splash sem login
// ✅ Emojis literais — sem surrogate pairs
...
async function solicitarPushPermissao(userId){
  ...
  try{
    ...
    if(token&&userId){
      ...
      const snap=await gDoc(fd(db,'barbeiros_auth',userId));
...
// ─── ALERTA DE AGENDAMENTO HOJE ───────────────────────────────
function AlertaAgendamento({agendamento,onFechar}){
  const[pisca,setPisca]=React.useState(true);
  React.useEffect(()=>{const i=setInterval(()=>setPisca(p=>!p),800);return()=>clearInterval(i);},[]);
  ...
  return(
    <div style={{position:'fixed',bottom:0,left:'50%',transform:'translateX(-50%)',width:'100%',maxWidth:'900px',zIndex:9999,background:pisca?'linear-gradient(135deg,#8B3A2A,#C9A84C)':'linear-gradient(135deg,#C9A84C,#8B3A2A)',transition:'background 0.4s',padding:'16px 20px',borderRadius:'0 0 20px 20px',boxShadow:'0 8px 32px rgba(0,0,0,0.6)'}}>
      ...
      <div style={{marginTop:'10px',display:'flex',gap:'8px'}}>
        <button onClick={()=>{const msg=encodeURIComponent(`Confirmando presença:\n✂️ ${agendamento.barbeiroNome}\n🕐 ${agendamento.hora}\nEstou a caminho! 👋`);window.open(`https://wa.me/5511977643509?text=${msg}`,'_blank');}} style={{flex:1,padding:'8px',borderRadius:'10px',border:'none',background:'#25D366',color:'#fff',fontSize:'12px',fontWeight:'700',cursor:'pointer'}}>
          📲 Confirmar presença
        </button>
...
function StaffLoginScreen({onBack,onSuccess,dark}){
  ...
  async function handleLogin(){
    ...
    try{
      ...
      if(!snap.exists()){setErro('Perfil não encontrado.');await signOut(auth);setLoad(false);return;}
      onSuccess({...snap.data(),uid:cred.user.uid});
    }catch(e){
      const msgs={'auth/user-not-found':'Email não encontrado.','auth/wrong-password':'Senha incorreta.','auth/invalid-email':'Email inválido.','auth/too-many-requests':'Muitas tentativas.','auth/invalid-credential':'Email ou senha incorretos.'};
...
function TelaRedefinirSenha({usuario,onConcluido,dark}){
  const s=getStyles(dark);
  const[senhaAtual,setSA]=React.useState('');
  const[senhaNova,setSN]=React.useState('');
  const[senhaConf,setSC]=React.useState('');
  const[erro,setErro]=React.useState('');
  const[loading,setLoad]=React.useState(false);
  ...
  async function handleRedefinir(){
    ...
    try{
      ...
      setOk(true);setTimeout(()=>onConcluido(),2000);
    }catch(e){
  ...
  if(ok)return<div style={{...s.app,display:'flex',alignItems:'center',justifyContent:'center',minHeight:'100vh'}}><div style={{textAlign:'center'}}><div style={{fontSize:'64px',marginBottom:'16px'}}>✅</div><div style={{fontFamily:"'Playfair Display',serif",fontSize:'20px',color:'#E8C96A'}}>Senha redefinida!</div></div></div>;
  return(
    <div style={{...s.app,padding:'40px 20px 24px'}}>
      <div style={{textAlign:'center',marginBottom:'32px'}}>
      ...
      {[{label:'Senha atual (temporária)',val:senhaAtual,set:setSA},{label:'Nova senha',val:senhaNova,set:setSN},{label:'Confirmar nova senha',val:senhaConf,set:setSC}].map(f=>(
      ...
      {erro&&<div style={{fontSize:'12px',color:'#F44336',marginBottom:'12px',textAlign:'center',background:'rgba(244,67,54,0.1)',borderRadius:'8px',padding:'10px'}}>{erro}</div>}
      <button style={{...s.btnGold,opacity:loading?0.7:1}} onClick={handleRedefinir} disabled={loading}>{loading?'⏳ Salvando...':'✅ Salvar nova senha'}</button>
    </div>
  );
}
...
// ─── SPLASH SCREEN ────────────────────────────────────────────
// ✅ Barbeiros carregados do Firestore | Clique abre FichaPublicaBarbeiro
  ...
  return(
    <div style={{...s.app,paddingBottom:'80px'}}>
      ...
      <div style={{padding:'20px 20px 0'}}>
        ...
        {barbeiros.map(b=>(
          // ✅ Card clicável — abre ficha pública sem login
          <div key={b.id} onClick={()=>onVerFicha&&onVerFicha(b)}
...
function TelaPromoJaResgatada({cliente,onIrParaHome,onAgendar,dark}){
  ...
  return(
    <div style={{...s.app,padding:'32px 20px',textAlign:'center'}}>
      ...
      <div style={{fontFamily:"'Playfair Display',serif",fontSize:'22px',color:'#E8C96A',marginBottom:'8px'}}>Você já garantiu sua vaga!</div>
      <div style={{fontSize:'14px',color:'#9A8880',marginBottom:'24px'}}>Olá, {cliente.nome.split(' ')[0]}! Este CPF já resgatou esta promoção.</div>
      <div style={{background:'rgba(139,0,0,0.2)',border:'1px solid rgba(244,67,54,0.3)',borderRadius:'14px',padding:'16px',marginBottom:'24px',textAlign:'left'}}>
        <div style={{fontSize:'12px',color:'rgba(255,255,255,0.6)',marginBottom:'4px'}}>Promoção registrada ✅</div>
        <div style={{fontSize:'12px',color:'#9A8880',marginTop:'4px'}}>Vá para agendamento para marcar seu horário</div>
      </div>
      <button onClick={onAgendar} style={{width:'100%',padding:'14px',borderRadius:'14px',border:'none',background:'linear-gradient(135deg,#8B0000,#F44336)',color:'#fff',fontSize:'15px',fontWeight:'700',cursor:'pointer',marginBottom:'10px'}}>
        📅 Agendar meu horário agora →
      </button>
      <button onClick={onIrParaHome} style={{width:'100%',padding:'12px',borderRadius:'12px',border:'1px solid #3A2018',background:'transparent',color:'#9A8880',fontSize:'13px',cursor:'pointer'}}>
        Ir para home
      </button>
    </div>
  );
...
function HomeCliente({cliente,onLogout,onNavegar,dark}){
  ...
  const[alertaVisivel,setAlerta]=React.useState(false);
  ...
  React.useEffect(()=>{
    ...
    getDocs(query(collection(db,'agendamentos'),where('clienteCpf','==',cliente.cpf),where('data','==',hojeStr),where('status','==','confirmado'))).then(snap=>{
      ...
      const prox=snap.docs.map(d=>d.data()).filter(a=>{
      ...
      if(prox){
        setLembrete(prox);
        const chave=`alerta_${cliente.cpf}_${prox.data}_${prox.hora}`;
        if(!localStorage.getItem(chave)){
          localStorage.setItem(chave,'1');setAlerta(true);tocarBip();
  ...
  return(
    <div style={{...s.app,paddingBottom:'80px'}}>
      {alertaVisivel&&lembrete&&<AlertaAgendamento agendamento={lembrete} onFechar={()=>setAlerta(false)}/>}
      <div style={{...s.header}}>
      ...
      <div style={{paddingTop:'4px'}}>
        <BannerPromocao cliente={cliente} onResgatado={()=>onNavegar('agendamento_promo')}/>
      </div>
      <div style={{padding:'16px 20px'}}>
        ...
        <div onClick={()=>setTour360(true)} style={{display:'flex',alignItems:'center',gap:'14px',padding:'14px',background:'rgba(232,201,106,0.06)',borderRadius:'14px',border:'1px solid rgba(232,201,106,0.2)',marginBottom:'10px',cursor:'pointer'}}>
...
export default function App(){
  ...
  const[configAbaInicial,setConfigAba]      = React.useState('horarios');
  const[promoParaResgatar,setPromoResgate]  = React.useState(null);
  const[fichaBareiro,setFichaBarbeiro]      = React.useState(null);
  const[deepLink,setDeepLink]               = React.useState(null);
  const[agDeepLink,setAgDeepLink]           = React.useState(null);
  const[authCarregando,setAuthCarr]         = React.useState(true);

  // ✅ Lê URL params para deep link (gerado pelo "Divulgar Horário Vago")
  React.useEffect(()=>{
    const params=new URLSearchParams(window.location.search);
    const bId=params.get('barbeiro'),bData=params.get('data'),bHora=params.get('hora');
    if(bId&&bData&&bHora){
        ...
        try{
          ...
          if(snap.exists()){
            ...
            if(!perfil.primeiroAcesso){
              // ✅ PONTO 1 — Cruzar email para obter id real do doc em 'barbeiros' (b1, b2, b3)
              let barbeiroDocId = null;
              if(perfil.email){
                try{
                  const snapB=await getDocs(query(collection(db,'barbeiros'),where('email','==',perfil.email)));
                  barbeiroDocId=snapB.docs[0]?.id||null;
  ...
  // ✅ Pós-login: deep link > promo > home normal
  async function handleLoginClienteSucesso(clienteLogado){
    setCliente(clienteLogado);
    if(deepLink){
      setAgDeepLink(deepLink);setDeepLink(null);
      setTela('agendamento_deeplink');return;
    }
    if(!promoParaResgatar){setTela('home_cliente');return;}
    try{
      const snap=await getDoc(doc(db,'resgates_promo',`${clienteLogado.cpf}_ativa`));
      if(snap.exists()){setTela('promo_ja_resgatada');return;}
    }catch(e){console.error(e);}
    setTela('agendamento_promo');
  }

  async function handleStaffLogin(u){
    // ✅ PONTO 2 — Cruzar email para obter id real do doc em 'barbeiros' (b1, b2, b3)
    let barbeiroDocId = null;
    if(u.email){
      try{
        const snapB=await getDocs(query(collection(db,'barbeiros'),where('email','==',u.email)));
        barbeiroDocId=snapB.docs[0]?.id||null;
      }catch(e){console.warn('[Login] Não achou doc barbeiros por email:',e);}
    }
  ...
  function handleNavegarGerente(modulo){
    const mapa={
      'config':           ()=>{setConfigAba('horarios');setTela('config_barbearia');},
      'servicos':         ()=>{setConfigAba('servicos');setTela('config_barbearia');},
    ...
    setEmBreveInfo({titulo:modulo});setTela('em_breve');
  }

  async function handleLogout(){
    try{await signOut(auth);}catch(e){}
    setUsuario(null);setCliente(null);setPromoResgate(null);setTela('splash');
  }
  ...
  return(
    <GlobalErrorBoundary>
      ...
      <div style={{minHeight:'100vh',position:'relative'}} className="app-container">
        {toggleTema}
        <ErrorBoundary modulo="App">
          ...
          {tela==='splash'&&(
            <SplashScreen
              ...
              dark={dark}
              onVerPromo={promo=>{setPromoResgate(promo);setTela('login_cliente');}}
              onVerFicha={barbeiro=>{setFichaBarbeiro(barbeiro);setTela('ficha_barbeiro');}}
            />
          )}
          ...
          {tela==='ficha_barbeiro'&&fichaBareiro&&(
            <FichaPublicaBarbeiro
              ...
              onAgendar={(barbeiro,modo)=>{
                setAgDeepLink({barbeiroPreSelecionado:barbeiro});
                if(cliente){setTela('agendamento');}
                else{setTela('login_cliente');}
              }}
            />
          )}

          {tela==='login_cliente'&&<ErrorBoundary modulo="LoginCliente"><LoginFlow onSucesso={handleLoginClienteSucesso} onBack={()=>{setPromoResgate(null);setTela('splash');}} dark={dark}/></ErrorBoundary>}

          {tela==='promo_ja_resgatada'&&cliente&&(
            <TelaPromoJaResgatada cliente={cliente} dark={dark}
              onIrParaHome={()=>{setPromoResgate(null);setTela('home_cliente');}}
              onAgendar={()=>setTela('agendamento_promo')}/>
          )}

          {tela==='home_cliente'&&cliente&&(
            <ErrorBoundary modulo="HomeCliente">
              <HomeCliente cliente={cliente}
                onLogout={()=>{setCliente(null);setPromoResgate(null);setTela('splash');}}
                onNavegar={t=>{if(t==='agendamento_promo')setTela('agendamento_promo');else setTela(t);}}
                dark={dark}/>
            </ErrorBoundary>
          )}
          ...
          {/* ✅ Agendamento tradicional — sem pré-agendamento */}
          {tela==='agendamento'&&cliente&&(
            <ErrorBoundary modulo="Agendamento">
              <Agendamento cliente={cliente} onBack={()=>setTela('home_cliente')} dark={dark} promoParaResgatar={null} deepLinkParams={agDeepLink||null}/>
            </ErrorBoundary>
          )}

          {/* ✅ Agendamento via deep link (WhatsApp do barbeiro) */}
          {tela==='agendamento_deeplink'&&cliente&&agDeepLink&&(
            <ErrorBoundary modulo="AgendamentoDeepLink">
              <Agendamento cliente={cliente} onBack={()=>setTela('home_cliente')} dark={dark} promoParaResgatar={null} deepLinkParams={agDeepLink}/>
            </ErrorBoundary>
          )}
          ...
          {tela==='config_barbearia'&&usuario&&<ErrorBoundary modulo="ConfigBarbearia"><ConfigBarbearia onBack={()=>setTela('home_gerente')} onVoltarAgenda={usuario.perfil===PERFIL.GERENTE?()=>setTela('agenda_barbeiro'):null} dark={dark} abaInicial={configAbaInicial}/></ErrorBoundary>}
