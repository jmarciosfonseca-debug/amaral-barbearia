...
// ✅ Passo 11: notificação WhatsApp ao cancelar
...
// ─────────────────────────────────────────────────────────────
// MODAL CANCELAR ✅ com notificação WhatsApp
// ─────────────────────────────────────────────────────────────
function ModalCancelar({ agendamento, onConfirmar, onFechar }) {
  ...
  async function handleCancelar() {
    ...
    try {
      ...
      setCancelado(true);
      // ✅ Abre WhatsApp para notificar barbearia
      setTimeout(() => {
        ...
        {cancelado ? (
          ...
          <>
            ...
            {ehHoje && (
              <div style={{ background: 'rgba(244,67,54,0.1)', border: '1px solid rgba(244,67,54,0.3)', borderRadius: '12px', padding: '12px 14px', marginBottom: '16px' }}>
                ...
                <div style={{ fontSize: '12px', color: '#F5EFE6', lineHeight: '1.6' }}>
                  Se remarcar para hoje, será necessário pagar um <strong>sinal de 50% via Pix</strong>.
                </div>
              </div>
            )}
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={onFechar} style={{ flex: 1, padding: '14px', borderRadius: '14px', border: '1px solid #3A2018', background: '#2E1A14', color: '#9A8880', fontSize: '14px', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>
                Manter reserva
              </button>
      ...
      <div style={{ padding: '14px' }}>
        ...
        {podeCancelar && (
          <div style={{ display: 'flex', gap: '8px' }}>
            <button onClick={() => onReagendar(ag)} style={{ flex: 1, padding: '10px', borderRadius: '10px', border: '1px solid #3A2018', background: '#2E1A14', color: '#E8C96A', fontSize: '13px', fontWeight: '600', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>
              🔄 Reagendar
            </button>
            <button onClick={() => onCancelar(ag)} style={{ flex: 1, padding: '10px', borderRadius: '10px', border: 'none', background: 'rgba(244,67,54,0.15)', color: '#F44336', fontSize: '13px', fontWeight: '600', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>
              ✗ Cancelar
            </button>
          </div>
        )}
    ...
    <div style={{ ...s.app, paddingBottom: '40px' }}>
      ...
      <div style={{ padding: '16px' }}>
        {carregando ? (
          ...
          lista.map(ag => (
            <CardAgendamento key={ag.firestoreId} ag={ag} onCancelar={ag => setCancelando(ag)} onReagendar={onReagendar} />
          ))
        )}
      </div>
